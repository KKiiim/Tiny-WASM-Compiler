...
void OP::get_r_param(uint32_t const paramIndex, bool const is64bit) {
  backend_.emit.append(str_r2ar_simm(ROP, static_cast<REG>(paramIndex), 0U, is64bit));
...
void OP::set_ofsp_local(uint32_t const offset2SP, bool const is64bit, bool const isTee = false) {
  ...
  assert(offset2SP <= MaxPositiveImmForLdrStr && "offset2SP too large");
  backend_.emit.append(ldr_simm_ar2r(REG::R9, ROP, 0U, is64bit));
  backend_.emit.append(str_r2ar_simm(REG::SP, REG::R9, static_cast<int32_t>(offset2SP & MaxPositiveImmForLdrStr), is64bit));
}
void OP::subROP(bool const is64bit) {
  backend_.emit.append(sub_r_r_imm(ROP, ROP, is64bit ? 8U : 4U, true));
}