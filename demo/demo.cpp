...
int main(int argc, char *argv[]) {
...

  {
    ...
    Runtime::CallReturn const ret =
        runtime.callByName<uint32_t>("div_s", "(ii)i", params[0], params[1], params[2], params[3], params[4], params[5], params[6]);
    if (ret.hasTrapped) {
      Trapcode const trapCode = runtime.getTrapCode();
      LOG_YELLOW << "TrapException caught with code: " << static_cast<uint32_t>(trapCode) << LOG_END;
